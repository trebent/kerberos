...
func TestAdminAPITLS(t *testing.T) {
	...
	resp, err := tlsClient(t).Get(fmt.Sprintf("https://localhost:%d/api/admin/flow", testlib.GetAdminPort()))
	if err != nil {
...
func TestAdminAPIPlainHTTP(t *testing.T) {
	...
	resp, err := plainClient().Get(fmt.Sprintf("http://localhost:%d/api/admin/flow", testlib.GetAdminPort()))
	if err != nil {
...
func TestGWAPITLS_mTLS_echo(t *testing.T) {
	...
	resp, err := tlsClient(t).Get(fmt.Sprintf("https://localhost:%d/gw/backend/mtls-echo/hi", testlib.GetPort()))
	if err != nil {
		t.Fatalf("HTTPS request failed: %v", err)
	}
	defer resp.Body.Close()
...
func TestGWAPITLS_TLS_echo(t *testing.T) {
	...
	resp, err := tlsClient(t).Get(fmt.Sprintf("https://localhost:%d/gw/backend/tls-echo/hi", testlib.GetPort()))
	if err != nil {
		t.Fatalf("HTTPS request failed: %v", err)
	}
	defer resp.Body.Close()
...
func TestGWAPIPlainHTTP(t *testing.T) {
	...
	resp, err := plainClient().Get(fmt.Sprintf("http://localhost:%d/gw/backend/mtls-echo/hi", testlib.GetPort()))
	if err != nil {
		t.Fatalf("Unexpected error when sending plain HTTP request: %v", err)
	}
	defer resp.Body.Close()
